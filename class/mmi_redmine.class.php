...
class mmi_redmine
{
	...
	public static function _activities_load()
	{
		...
		//var_dump(static::$_activities);

		//return static::$_activities;
	}
	 ...
	 */
	public static function sync_last($project_ids=[])
	{
		global $db, $user;
	 ...
	 */
	public static function sync_r_project($r_project, $date_from=NULL, $autocreate=true)
	{
		global $user, $db;
		var_dump($user);
		...
		$r_time_entries = static::api_time_entries(['project_id'=>$r_project->id, 'date_from'=>$date_from, 'offset'=>0, 'limit'=>1000]);
		foreach($r_time_entries as $r_time_entry) {
			...
			$time_entry->note = $r_time_entry->description;
			$time_entry->thm = 35;
