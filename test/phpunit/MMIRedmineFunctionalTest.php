...
/**
 * \file    test/phpunit/MMIRedmineFunctionalTest.php
 * \ingroup mmiredmine
...
/**
 * Class MMIRedmineFunctionalTest
 *
 ...
 *
 * @package Testmmiredmine
 */
class MMIRedmineFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modMMIRedmine')->click();
		} else {
			// Disable the module
			$this->byHref('modMMIRedmine')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/mmiredmine/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('mmiredmine/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/mmiredmine/admin/about.php');
		$this->authenticate();
		return $this->assertContains('mmiredmine/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('mmiredminewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMMIRedmine_MMIRedmineTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMMIRedmine_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
